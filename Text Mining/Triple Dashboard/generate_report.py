# generate_report.py (Version 2.2)
import json
import argparse
...
def generate_html_report(json_path: str, output_path: str) -> None:
    ...
    try:
        ...
        # First, create the unified list and sort it
        all_entity_types_sorted = []
        for entity_name, total_count in entity_type_counts.items():
            utterance_count = len(entity_utterance_tracker.get(entity_name, set()))
            all_entity_types_sorted.append({
                "name": entity_name,
                "count": total_count,
                "utterance_count": utterance_count
            })
        all_entity_types_sorted.sort(key=lambda x: x["count"], reverse=True)
        print(f"[*] Counted {len(all_entity_types_sorted)} unique entity types.")

        # ▼▼▼ NEW: Categorize entity types by utterance frequency ▼▼▼
        entity_types_high_freq = []
        entity_types_medium_freq = []
        entity_types_low_freq = []
        for etype in all_entity_types_sorted:
        ...
        entity_utterance_counts_map = {item['name']: item['utterance_count'] for item in all_entity_types_sorted}
        entity_total_counts_map = {item['name']: item['count'] for item in all_entity_types_sorted}
        ...
        for rel in all_rels:
            ...
            relation_diversity.append({'rel': rel, 'domain_size': domain_size, 'range_size': range_size, 'total_diversity': domain_size + range_size})
        ...
        global_stats = {
            ...
            "unique_entity_types": len(all_entity_types_sorted),
        ...
        html_content = template.render(
            report_title="Relation Extraction Dashboard",
            ...
            one_to_many_relations=sorted(list(one_to_many_relations)),
            many_to_one_relations=sorted(list(many_to_one_relations)),
